...
from collection_creation.fileIO import matches, norm_word, zone
from converter.postings_converter import from_gamma_str, to_gamma_str
...
def block_from_str(m_str, encoding):
    word_len = m_str[0]
    common_part = m_str[1: 1 + word_len].decode(encoding)
    m_str = m_str[1 + word_len:]
    terms = []

    while m_str:
        word_len = m_str[0]
        terms.append(common_part + m_str[1: 1 + word_len].decode(encoding))
        m_str = m_str[1 + word_len:]
    return terms
...
class Index:
    ...
    def zone_query(self, query):
        words = set(query.split(' '))
        vocab = {}
        for word in words:
            for rank, id in  self.get_postings(word):
                if id not in vocab:
                    vocab[id] = {'author': [], 'title': [], 'body': [], 'rank': 0}
                vocab[id]['rank'] += rank
                add_zone(vocab[id], 'author', rank, word)
                add_zone(vocab[id], 'title', rank, word)
                add_zone(vocab[id], 'body', rank, word)
        return sorted(([key,val] for key,val in vocab.items()),  key=lambda x: x[1]['rank'], reverse=True)


def add_zone(elem, zone_name, rank, word):
    if rank & zone(zone_name):
        elem[zone_name].append(word)
